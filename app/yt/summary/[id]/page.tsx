...
function TypewriterText({ text, isAnimating, className }: { text: string; isAnimating: boolean; className?: string }) {
    ...
    return (
        <span className={className}>
            {displayText}
            {isTyping && <span className="inline-block w-[2px] h-[1em] bg-blue-500 ml-0.5 animate-blink align-text-bottom rounded-sm" />}
        </span>
    )
}

// ─── Gemini TTS helper — plays text via /api/tts ───
...
function InlineShimmer({ rect }: { rect: DOMRect }) {
    return (
        <div className="fixed pointer-events-none z-[99]" style={{ left: rect.left - 3, top: rect.top - 3, width: rect.width + 6, height: rect.height + 6 }}>
            <div className="w-full h-full rounded-md border-2 border-blue-400/50 bg-blue-400/8 animate-pulse" />
        </div>
    )
}
...
export default function SummaryPage() {
    ...
    // Transcript translation (separate)
    ...
    // Helper: save translation state to Supabase
    ...
    useEffect(() => {
        async function load() {
            ...
            // Restore persisted translations from Supabase
    ...
    // Word selection listener — captures cursor position for popup placement
    useEffect(() => {
    ...
    // Full page translation (excludes transcript)
    ...
    // Separate transcript translation (chunked)
    ...
    // TTS handler (Gemini TTS via /api/tts)
    const handleTTS = useCallback(() => {
        ...
        playTextWithGeminiTTS(
            ...
            () => { setIsSpeaking(false); setIsGeneratingTTS(false) },
            { pageId: id, pageTitle: data?.videoTitle, section: "tts-summary" },
            () => setIsGeneratingTTS(true),
        )
    }, [data, translatedData, isSpeaking, isGeneratingTTS, pageLang])
        ...
        return (
            <div className="min-h-screen bg-background flex items-center justify-center">
                <div className="text-center">
                    ...
                    <button onClick={() => router.push("/workspace")} className="text-blue-500 hover:underline">← Back to Workspace</button>
                </div>
            </div>
        )
    }
    ...
    return (
        <div className="min-h-screen bg-background">
            ...
            <main className="max-w-4xl mx-auto px-6 py-12">
                ...
                {/* Meta */}
                <div className="flex flex-wrap items-center gap-3 text-sm text-zinc-500 mb-10">
                    <span>{data.channel}</span>
                    <span className="text-zinc-300 dark:text-zinc-700">•</span>
                    <span>{new Date(data.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" })}</span>
                    <span className="text-zinc-300 dark:text-zinc-700">•</span>
                    <a href={data.videoUrl} target="_blank" rel="noopener noreferrer" className="text-blue-500 hover:underline">Watch on YouTube ↗</a>
                    {pageLang !== "en" && (
                        <>
                            <span className="text-zinc-300 dark:text-zinc-700">•</span>
                            <span className="text-blue-500 text-xs font-medium bg-blue-50 dark:bg-blue-500/10 px-2 py-0.5 rounded-full">{currentLang?.flag} {currentLang?.name}</span>
                        </>
                    )}
                </div>
                        ...
                        <div className="space-y-3">
                            {displayKeyPoints.map((kp: any, i: number) => (
                                <div key={i} className="flex items-start gap-3">
                                    <span className="font-mono text-sm text-blue-500 mt-0.5 shrink-0 bg-blue-50 dark:bg-blue-500/10 px-2 py-0.5 rounded">{kp.timestamp}</span>
                                    ...
                                    <div className="grid grid-cols-2 gap-1 mt-1">
                                        {LANGUAGES.map((lang) => (
                                            <button key={lang.code} onClick={() => handleTranscriptTranslation(lang.code)}
                                                className={`flex items-center gap-2 px-2 py-1.5 text-xs rounded-lg transition-all ${transcriptLang === lang.code ? "bg-blue-50 dark:bg-blue-500/10 text-blue-600 dark:text-blue-400 font-medium" : "hover:bg-zinc-50 dark:hover:bg-zinc-800 text-foreground/80"}`}>
                                                <span>{lang.flag}</span>
                                                <span className="truncate">{lang.name}</span>
                                            </button>
                                        ))}
                ...
                {displayTTS && (
                    <section className="mb-12">
                        <div className="flex items-center gap-2 mb-4">
                            ...
                            <span className="text-[10px] text-blue-500 bg-blue-50 dark:bg-blue-500/10 px-2 py-0.5 rounded-full ml-2 uppercase font-medium">AI Rewritten</span>
                        </div>
                        <div className="bg-gradient-to-br from-blue-50/50 to-zinc-50 dark:from-blue-500/5 dark:to-zinc-900/50 rounded-xl p-6 border border-blue-100 dark:border-blue-500/10">
                            ...
                            <button onClick={handleTTS}
                                ...
                                className={`inline-flex items-center gap-2 px-5 py-2.5 rounded-xl text-sm font-medium transition-all ${isGeneratingTTS
                                    ? "bg-gradient-to-r from-blue-400 to-blue-600 text-white shadow-lg shadow-blue-500/20 animate-pulse cursor-wait"
                                    : isSpeaking
